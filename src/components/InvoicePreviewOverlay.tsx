    ...
    <div className="fixed inset-0 z-[200] flex items-start justify-center p-4 bg-slate-900/60 backdrop-blur-md overflow-y-auto invoice-print-parent print:static print:block print:w-full print:h-auto print:bg-white print:p-0">
      <div className="bg-white w-full max-w-[850px] shadow-2xl rounded-[10px] overflow-hidden my-8 invoice-print-container">
        ...
        {(() => {
          ...
          return chunkedItems.map((pageItems, pageIdx) => {
            ...
            return (
              <div 
                key={pageIdx} 
                className={cn(
                  "p-6 md:p-8 bg-white min-h-[1050px] print:min-h-0 text-slate-900 leading-normal border-[1px] border-slate-800 m-2 md:m-4 flex flex-col justify-between relative invoice-page",
                  pageIdx < totalPages - 1 ? "mb-8 pb-8 border-b-2 border-dashed border-slate-300 print:mb-0 print:pb-0 print:border-none" : ""
                )}
              ...
              >
                      ...
                      <table className="w-full text-left text-[12px] border-collapse">
                              ...
                              <tr key={item.id || globalIdx} className="border-b-[1.5px] border-slate-200">
                                ...
                                <td className="p-1 border-r-[1.5px] border-slate-800 font-black uppercase text-[11px] whitespace-normal break-words leading-tight">
                                  <EditableInvoiceField
                                    ...
                                    onChange={(val) => updatePreviewItem(globalIdx, 'modelName', val)}
                                    as="textarea"
                                    rows={2}
                                ...
                                <td className="p-1 border-r-[1.5px] border-slate-800 text-center">
                                  <EditableInvoiceField
                                    ...
                                    onChange={(val) => updatePreviewItem(globalIdx, 'hsn', val)}
                                ...
                                <td className="p-1 border-r-[1.5px] border-slate-800 text-right pr-2">
                                  <EditableInvoiceField
                                    type="number"
                                    value={item.quantity}
                                    onChange={(val) => updatePreviewItem(globalIdx, 'quantity', parseFloat(val) || 0)}
                                ...
                                <td className="p-1 border-r-[1.5px] border-slate-800 text-right pr-2">
                                  <EditableInvoiceField
                                    ...
                                    onChange={(val) => updatePreviewItem(globalIdx, 'unitPrice', parseFloat(val) || 0)}
                                    className="text-right font-black text-[11px] bg-transparent p-0 border-none h-auto outline-none"
                                  />
                                </td>
                        ...
                        <tfoot>
                          <tr className="bg-[#fcf8ef] text-slate-900 font-black text-center border-t-[1.5px] border-b-[1.5px] border-slate-800 uppercase">
                            <td colSpan={3} className="p-2 border-r-[1.5px] border-slate-800 text-right pr-4 font-black">
                              {isLastPage ? 'TOTAL :' : 'PAGE TOTAL :'}
                            </td>
                ...
                {/* Declaration (Outside the main border box, but at the absolute bottom of the page) */}
                <div>
                  <div className="mt-2 print:mt-1 border-[1px] border-slate-800 text-[10px] p-2 flex items-center justify-between bg-slate-50/20">
                    ...
                    <p className="font-bold">Page No : {pageIdx + 1} of {totalPages}</p>
                  </div>
